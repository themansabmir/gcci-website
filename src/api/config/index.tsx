...
api.interceptors.request.use(
  (config) => {
    const token = localStorage.getItem(constants.TOKEN) || localStorage.getItem("gcci_auth_token");
    if (token) {
  ...
  (error) => {
    ...
    switch (response.status) {
      case 401:
        ...
        localStorage.removeItem("gcci_auth_token");
        localStorage.removeItem("gcci_customer");
